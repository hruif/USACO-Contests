...
int main() {
    ...
    vector<vector<vector<ll> > > dp(K + 1, vector<vector<ll> >(N, vector<ll>(sa.size(), 1000000000)));
    ...
    for (int i = 1; i <= K; i++) {
        ...
        for (int j = 1; j < N; j++) {
            ...
            for (int k = 0; k < sa.size(); k++) {
                if (sa[k] >= a[j]) {
                    dp[i][j][k] = min(pmax[j - 1], dp[i][j - 1][k]) + sa[k] - a[j];
                    npm = min(npm, dp[i][j][k]);
                }
            }
            pmax[j - 1] = pnpm;
            pnpm = npm;
